...
# Parsed by parse_sample_name() below — no separator needed.

# Which condition labels identify untreated/control samples?
control_labels <- c("Plasmid")  # empty vector controls (26 replicates)
...
# Reporter backgrounds to exclude (not applicable for M. smegmatis)
exclude_reporters <- c()

# Name corrections (none yet)
name_corrections <- list()

# Replicate groups (none yet — replicates handled by parse_sample_name)
replicate_groups <- list()
merge_replicates <- FALSE

# Include control samples in UMAP?
include_controls_in_umap <- TRUE

# Include reporter fluorescence as an additional feature?
include_fluorescence <- FALSE
fluorescence_col     <- "INTENSITY.ch1.mean"

# Drug conditions are identified by experiment_type == "drug" from parse_sample_name
drug_experiment_pattern <- "drug"

# Gene name annotation: map MSMEG accessions to gene names for display
...
# Parse M. smegmatis sample names into components.
# Raw EXPERIMENT values may include prefixes (Labelled__, Labelled__Drugs__)
# which are stripped before matching:
#   Control: Plasmid_RY           → experiment_type="control", knockdown="Plasmid"
#   Mutant:  MSMEG_XXXX_RY       → experiment_type="mutant",  knockdown=MSMEG_XXXX
#   Drug:    ZZZ_AX_RY            → experiment_type="drug",    knockdown=ZZZ_AX
...
# Load gene name annotations (MSMEG accession → gene name for display)
if (file.exists(annotation_file)) {
  ...
  message(sprintf("Gene name annotations loaded: %d/%d mutants have gene names",
                  sum(unique(cell_data$knockdown) %in% names(gene_name_map)),
                  n_distinct(cell_data$knockdown[cell_data$experiment_type == "mutant"])))
} else {
  cell_data <- cell_data %>% mutate(display_label = knockdown)
  message("No annotation file found — using accession numbers as labels")
}
...
# INTENSITY.ch1.mean reflects the ParB fluorescence signal.
# Not applicable for M. smegmatis in the same reporter-specific way as Mtb,
# but the normalisation logic is preserved in case it's useful.
...
if (include_fluorescence && fluorescence_col %in% names(cell_data)) {
  ...
  # Each reporter becomes its own pair of columns (e.g. reporter_fluor_mean, ...)
